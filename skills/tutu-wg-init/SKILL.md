...
- 密钥一律由服务器生成并当场注册（客户端不自行生成密钥，见客户端模式 C2）；**签发完成后必须当场输出完整客户端配置提示词（S5 强制要求，含 `.tu` 域名解析配置项），未输出即流程未完成**
...
**强制要求**——签发（新增/换公钥）完成后必须当场输出以下完整提示词，输出前流程不得结束。提示词必须逐项包含:

1. 成品客户端配置（PrivateKey / Address 为变量替换后的最终值，非占位符）
2. **`.tu` 域名解析两行**: `PostUp = resolvectl dns wg0 192.168.125.1; resolvectl domain wg0 '~tu'` 与对应 `PreDown` 行——tutu 内网的名字解析完全依赖客户端这两行，漏掉则新机器 `*.tu` 域名全部不可解析（VPN 通了也用不了域名）
3. 目标机器无 resolvectl 时删掉这两行的说明
4. 带回目标机器 pi 会话校验落盘的指引
...
换公钥（私钥丢失重建）同样属于签发: **必须**按上述强制要求输出新的客户端配置提示词（同新增格式，仅 PrivateKey/Address 按新值，含 `.tu` 域名解析两行），并提示旧私钥已作废、目标机器按片段重写 wg0.conf 后重启 wg。
...
- 签发（新增/换公钥）后未输出完整提示词，或提示词缺 `.tu` 域名解析两行（PostUp/PreDown）→ 流程未完成，必须补齐输出后才能结束
- 服务器模式 IP 非法 / 是 `.1` / 已占用 / nickname 含非法字符 → 重新询问，不写配置
